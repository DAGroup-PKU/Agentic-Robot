...
def build(base_path="", output_dir=None):
    ...
    def post_card(p, filterable=False):
        ...
        return f'''<article class="post-card"{attrs}>
          {media}
          <div class="blog-card-copy">{post_meta(p)}<h3><a href="/posts/{p['slug']}/">{esc(p['title'])}</a></h3><p>{esc(p['excerpt'])}</p><div class="blog-card-actions"><a class="text-link" href="/posts/{p['slug']}/">Read Blog <span aria-hidden="true">↗</span></a>{external(p.get('results_label', 'Interactive Webpage'), p['results_path'])}</div></div>
        </article>'''
    ...
    for p in projects:
        ...
        page(f"projects/{p['slug']}/index.html", p["name"], p["description"], f'''
          <section class="wrap project-intro"><a class="back-link" href="/projects/">← All projects</a><div class="eyebrow">Project {p['number']} / {esc(p['environment'])}</div><h1>{esc(p['name'])}</h1><p class="lede">{esc(p['description'])}</p><div class="project-actions">{external(p.get('results_label', 'Open interactive results'), p['results_path'], 'button dark')}<span class="mode">{esc(p['mode'])}</span></div></section>
    ...
    for p in posts:
        ...
        cta = {"eyebrow": "From the story to the evidence", "title": "Inspect the experiment.",
               "text": "Explore the task-level results, methods, and recordings.", "button": "Open interactive report",
               **p.get("cta", {})}
        ...
        page(f"posts/{p['slug']}/index.html", p["title"], p["excerpt"], f'''
          <header class="wrap article-header"><a class="back-link" href="/blogs/">← All blogs</a>{post_meta(p)}<h1>{esc(p['title'])}</h1><p class="lede">{esc(p['subtitle'])}</p><div class="article-header-actions">{external(p.get("results_label", "Interactive Webpage"), p["results_path"], "button dark")}</div><div class="byline"><div class="author-byline"><span class="author-label">Authors</span><div class="author-list">{''.join('<span class="author-name">' + esc(author) + '</span>' for author in p.get('authors', [site['name']]))}</div>{author_note}</div><button class="copy-link" data-copy>Copy link <span aria-hidden="true">↗</span></button><span class="sr-only" data-copy-status aria-live="polite"></span></div></header>
          <div class="wrap article-layout"><article class="prose">{body}<div class="article-cta"><div class="eyebrow">{esc(cta['eyebrow'])}</div><h3>{esc(cta['title'])}</h3><p>{esc(cta['text'])}</p>{external(cta['button'], p['results_path'], 'button dark')}</div>{article_citation(p, site)}</article><aside class="article-aside"><div class="sticky"><div class="eyebrow">In this article</div><nav class="toc" aria-label="Table of contents">{toc}</nav><div class="eyebrow">{esc(p.get('aside_label', 'Interactive report'))}</div>{report_links}</div></aside></div>
          {related_section}
        ''', "blogs", "article-page")
